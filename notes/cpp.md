...
<!-- vim-markdown-toc -->
&emsp;混合了C++11/14/17/20的标准，C++20的大部头并没有涉及(模块、概念、协程)  ，
文中会有标记写出哪版标准支持该特性，但无标记可不代表都支持哦，
因为。。我忘了时哪版支持的了，还是让编译期告诉我们吧  
&emsp;本笔记有[精简版](http://note.youdao.com/noteshare?id=7e236a03f9b1f9338a46c792fb7c2a19)用于快速浏览和回忆
...
### 关于链接
* const与constexpr：  
    被这两个关键字修饰后，默认不对外，可以使用**extern**强制对外
* extern：  
    告诉编译器，该对象是引用其它文件中的定义，否则会在本文件定义该对象并记录在符号表中，
    若其它翻译单元(TU)也定义了该符号则链接报错
...
* 对类的static数据成员：
    * 同全局变量，将所有**TU**的同名`inline`变量链接到同一定义，且`constexpr`会隐式`inline`
    * 可以在类内定义`static constexpr`数据成员，而无需再在类外定义一次以形成实体  
...
* 构造函数会调用其数据成员的构造函数，当类的嵌套愈来愈深，构造函数的代码重复便会愈来愈严重
>
...
* operator：  
    * 重载运算符
        > 特殊的有`operator new`、`operator""s`等
...
* return：
    ...
    * 默认使用移动构造返回对象
    * 返回类型推断：`auto`与`deltype(auto)`
        * 所有返回类型均要相同（可以使用`if constexpr`绕过）
        * 且递归调用之前必须有return以确定返回类型
>
...
* 重载函数
    ...
    * 模板函数若推断某实例符合则也纳入候选函数，更[特例化](#px)的优先
...
* move语义
    * `ClassName(ClassName&&): initlist {funcbody}`
    * move自赋值： 直接拒绝自赋值
    * 若不指定为`noexcept`，则标准库容器不会使用移动构造函数
>
...
* 类类型转换
    ...
    * 二义性问题：
        * 有多个算术类型的转换函数，转换为确定的类型应该调用哪个转换函数？
            > &emsp;类与算数类型之间的转换，会发生额外的标准类型转换（只能在标准类型之间转换），
            > 最佳匹配便取决于此  
            > &emsp;包括：精准转换、整型提升、浮点转换
        * 有一个算术转换函数和一个其它类型的转换函数，重载函数应该调用哪个类的版本？
            > 所有用户级别的转换的优先级都相同，故产生二义性  
            > **但是当只有接受标准类型的候选函数时，精准转换更优，整型提升与浮点转换同等次优**
    * 经验：
        * 不要在两个类之间定义相同方向的转换
        * 最多定义一个与算术类型的转换(除了bool)，让编译期自动进行内置类型的转换，且最好全部explicit
...
* 聚合初始化： <span id="jhcsh"></span>
    ...
    * C++20支持在花括号中使用指示符以初始化确定的成员，但荏苒需要保持顺序  
        > `{.a_m = 1, b_m{2}}`，其中`a_m`在`b_m`之前构造
    * 注：与普通的类不同，聚合类与数组类似，默认构造函数不会进行[值初始化](https://github.com/mrbeardad/DotFiles/blob/master/notes/cppstyle.md#%E5%88%9D%E5%A7%8B%E5%8C%96)
...
* 分支判断
    ...
    * `else`
    * `if constexpr ( ; )`
        > 一般用于SFINAE以推断函数返回的auto，或std::visit(func, variant)设计func
* 开关多分支
    * `switch( ; )`
    * `case IntConstexpr :`
    * `default :`
...
* 作用：
    ...
    * 用于隐式构造为需要的类型（**需要non-explicit构造** ） `test({"implicitly convert to string"})`
>

### 类型转换
> 不鼓励使用旧式的C-style-cast，下面的新式转换更容易在文件中搜索，且更容易由编译期帮助检测
* `static_cast<>()`         ：一般用于将`void*`转换为具体类型的指针等允许但非显式的转换
* `const_cast<>()`          ：用于取消底层const
* `reinterpret_cast<>()`    ：一般用于转换指针类型
* `dynamic_cast<>()`        ：用于在继承体系中转换它们的指针或引用
>
...
* 继承体系
    * 每个含有virtual函数的类对象，都在其内部存储有虚指针指向虚表
    * 派生类分成两部分：基类部分，派生部分
    * 访问基类部分需要通过基类接口
        * 派生类：
...
## 虚函数
> 基类需要有虚函数，并且其析构函数很有必要声明为virtual<span id="xhu"></span>  
...
* 函数模板的偏序：用于判断重载的模板函数中哪个更特例化 <span id="px"></span>
    ```cpp
    template<typename T>
