...
### 关键字
* const与constexpr：  
    默认不对外，可以使用**extern**强制对外
* extern：  
    告诉编译器，该对象是引用其它文件中的定义，否则会在本文件定义该对象并记录在符号表中，
    若其它翻译单元(TU)也定义了该符号则链接报错

> 关于引用动态库中的vector的思考：  
> vector需要堆内存，故它的构造需要在运行期调用构造函数。  
> &emsp;如果在main函数所在文件定义`vector<string>`，那在main中引用它很简单，进入main前构造`vector<string>`即可  
> &emsp;如果在要引用静态库中的`vector<string>`，那和上面情况一样  
> &emsp;但如果是引用动态库中的呢？数据引用的解析会拷贝动态库的数据段，可`vector<string>`需要堆内存呀！而动态库可没有，也不会去构造`vector`。  
> &emsp;一个猜想是在进入main前，解析数据引用后调用其构造函数。因为在main所在源文件中就已经知道要引用别处的`vector`，
> 而`vector`是需要构造的，故等到数据引用绑定完毕即可进行构造。  
> 当然以上仅吾拙见，留待以后学到了纠正吧
...
* 对函数：
    ...
    * 若展开失败则保证所有同名inline链接到同一定义
        > 存在多个不同的定义是链接不会报错，而是链接到其中一个，Linux + GCC平台是链接到第一个参数的文件中的定义，
        > C++标准未定义，也不应对此有期望
* 对全局变量：
    * 将所有编译单元(**TU**)的同名inline变量链接到同一定义
        > 作用是可以在头文件中定义inline static const变量优化而不引起符号重定义的链接错误
* 对类的static数据成员：
    * 同全局变量，将所有编译单元(**TU**)的同名inline变量链接到同一定义
        > 可以在类内定义static constexpr数据成员，而无需再在类外定义一次以形成实体  
        > 为模板类提供了定义static数据成员的机制，避免使用[控制实例化](#kzslh)的缺陷与麻烦
* 对函数模板与变量模板：
    * 它们具有隐式inline属性。不同的是，不是提示编译器内联展开，只是将多个符号定义链接为同一个
>
<a href=## title="参考自 Effective C++ 3e 条款30">[注]</a>：莫要滥用inline函数，特别是构造函数与析构函数，因为
* 构造函数的异常安全性，会捕获成员构造所抛出的异常并析构已构造的成员，则会有大量的重复调用成员的析构函数，
    特别是第一个构造的成员的析构函数
* 若基类的构造函数为inline，则它的所有派生类的构造函数调用它时都会重复一遍其执行代码，随着继承树愈来愈深，
    若构造函数都是inline，那带来的代码膨胀可能是无法接收的
>
...
* static变量
    ...
    * 用于reference-returning技术
>

* 默认实参
    * 若某一形参有默认实参，则其后的形参也必须有
    * 大概率不会被使用的默认实参的形参则应排在前面
    * 声明与定义分离时，定义式不能重复指出默认实参
>
...
* 析构函数 <span id="xghu"></span>
    * `~classname() {}`
    * 逆序销毁
    * noexcept：析构函数不应抛出异常，或者说，不应该让异常逃离析构函数（在内部解决它）
