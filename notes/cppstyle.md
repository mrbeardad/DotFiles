...
* 类的数据成员&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;：依据以上两写法并带`_m`后缀`exampleName_m`
...
* 内置类型初始化与调用非copy非move构造函数函数时才使用列表初始化`{args}`，否则用等号`=`
...
* 类之间的关系及其设计
    * D ***is-a*** B：B的所有特性与操作D都支持。例如，"student is a person"
        > 设计：D <u>public继承</u> B。若B的某些操作D不支持，则从B特化派生（is-a）出适合的B1供D进行public继承
    * D ***has-a*** B：B是D的一个组分或一种性质。例如，"person has a name and ID"
        > 设计：D <u>复合</u> B，即B作为D的数据成员
    * D ***is-implement-in-terms-of*** B：B为D提供一些用户不可见的实现细节
        > 设计：
        > * D <u>复合</u> B
        > * D <u>priveta继承</u> B，一下情况才选择此设计：
        >   * D需要访问B的protect成员
        >   * D需要修改B的virtual函数
        >   * 利用EBO优化基类尺寸
* 继承基类时的接口含义
...
* 数据成员应该封装为private，为了更好的封装数据成员以降低编译依赖甚至可以使用**handle类**或**interface类**
...
* 将多态基类的析构函数声明为virtual，注意声明virtual析构函数的同时会拒绝编译器合成默认析构函数，
    故需手动定义或者使用`=default`；若不是多态基类则不要声明任何虚函数而引入虚指针与虚表
* 不要在构造函数与析构函数中调用虚函数

## 泛型编程
* 减少代码膨胀
    * 将与模板参数无关的代码从类模板与函数模板中抽离
    * 与非类型参数有关的代码应该使用一份共享的相同的实现代码
...
* 默认初始化与值初始化
    * 对于内置类型与聚合类，`int a[5];`叫默认初始化，`int a[5]{}`叫值初始化，  
        默认初始化会调用其默认构造函数，但其默认构造函数的行为是未定义的，它初始化静态变量为全零，初始化动态变量为未定义；  
        而值初始化会将初始化对象为全零(0)
