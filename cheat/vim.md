...
| `<c-a>与<c-e>`         | 行首，行尾                                    |
| `<c-d>与<c-b>`         | 向下翻半页，向上翻半页                        |
| `<c-f>与<c-u>`         | 向下翻一页，向上翻一页                        |
| `,`                    | 复制光标下字符                                |
| `Y`                    | 复制到行尾                                    |
| `<leader>y`            | 复制到系统剪切板`"+y`                         |
| `<leader>,`            | 复制直到行尾到系统剪切板`"+yl`                |
| `<leader>Y`            | 复制直到行尾到系统剪切板`"+y$`                |
| `<space>bY`            | 复制buffer至系统clipboard                     |
| `<leader>p与<leader>P` | 在光标前/后粘贴系统剪切板                     |
| `<leader>o与<leader>O` | 在下/上行粘贴系统剪切板                       |
...
| `<c-w>-与<c-w>+`          | 减少与增加窗口高度        |
| `<c-w><与<c-w>>`          | 减少与增加窗口宽度        |
| `{num}<c-w>_与{num}<c-w>` | 设置窗口高度与宽度为{num} |
| `<c-w>c`                  | 关闭窗口                  |
| `<c-w>o`                  | 关闭其它所有窗口          |
| `<c-w>s`                  | 水平分割窗口              |
| `<c-w>v`                  | 垂直分割窗口              |
...
| `<space>ac` | 计算器                       |
| `<space>al` | 日历                         |
| `<space>af` | 从当前文件所在目录打开文件树 |
| `<space>as` | Startify启动界面             |
| `<space>ar` | 阅读模式                     |
| `<space>aR` | 选择一个窗口进入阅读模式     |
<!-- -->
## 制表与对齐
* 制表与对齐：
...
> 其它同理。替换的字符(第二个{char})若是`<`，则还可以继续输入来添加html标签  
